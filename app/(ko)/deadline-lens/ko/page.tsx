...
const privacy = ["기기 내 처리", "계정 없음", "광고 없음", "광고 추적 없음", "클라우드 동기화 없음", "Google Play 결제"];
...
export default function DeadlineLensKoPage() {
  return (
    <main lang="ko">
      ...
      <section className="bg-zinc-950 text-white"><div className="mx-auto grid max-w-6xl gap-10 px-6 py-16 lg:grid-cols-[1fr_1.2fr] lg:items-center"><div><p className="mb-2 text-sm font-semibold text-indigo-300">개인정보 보호 우선</p><h2 className="text-3xl font-bold tracking-tight">문서는 기기 밖으로 나가지 않습니다</h2><p className="mt-4 leading-7 text-zinc-300">이미지, 인식한 텍스트 및 추출 결과는 기기 안에서 처리됩니다. Deadline Lens는 구매 제공과 복원을 위해서만 Google Play 및 RevenueCat을 사용합니다. 계정, 광고, 광고 추적 또는 클라우드 동기화 기능은 없습니다.</p></div><div className="grid gap-3 sm:grid-cols-2">{privacy.map((item) => <div key={item} className="rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-4 text-sm font-semibold"><span className="mr-2 text-indigo-300">✓</span>{item}</div>)}</div></div></section>
