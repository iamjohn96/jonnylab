...
const sections = [
  { title: "소개", content: <p>본 개인정보 처리방침은 JonnyLab이 사진 속 잠재적인 민감정보를 찾고 가릴 수 있도록 돕는 Android 및 iPhone 앱 Privune(이하 “Privune” 또는 “앱”)과 관련된 정보를 처리하는 방식을 설명합니다. Privune은 온디바이스 처리, 내보내기 전 사용자 검토, 별도의 안전한 사본(Safe Copy) 생성을 중심으로 설계되었습니다.</p> },
  ...
  { title: "데이터 보유 및 삭제", content: <><p>OCR 텍스트, 탐지된 민감정보, 바운딩 박스 및 편집 상태는 진행 중인 작업을 위해 로컬로 사용되며 JonnyLab이 보유하지 않습니다. 저장된 안전한 사본은 사용자가 삭제할 때까지 사진 보관함이나 파일에 남습니다.</p><p>Google, Apple 및 RevenueCat은 자체 방침과 법적 의무에 따라 구매 기록을 보유할 수 있습니다. Privune 또는 JonnyLab이 관리하는 RevenueCat 연결 데이터의 삭제를 요청하려면 <a href="/privune/data-deletion">Privune 데이터 삭제 요청 페이지</a>의 절차를 따라주세요.</p></> },
...
export default function PrivunePrivacyKoPage() {
  return <PrivuneLegalPage lang="ko" title="개인정보 처리방침" effectiveDate="시행일 및 최종 수정일: 2026년 8월 18일" alternateHref="/privune/privacy" alternateLabel="View in English" sections={sections} otherDocumentHref="/privune/terms/ko" otherDocumentLabel="이용약관" />;
}
